...
const ColourfulList = ({ items, showDelete, ...props }) => {
  return (
    <ul className="colourful-list">
      {items.map((item, index) => {
        ...
        return (
          <li key={index} style={{ background: item.bg, color: item.fg }}>
            {item.title}
            <UpButton onClick={() => props.onMove(index, prevIndex)} />
            <DownButton onClick={() => props.onMove(index, nextIndex)} />
            {showDelete && <DeleteButton onClick={() => props.onDelete(index)} />}
          </li>
        );
      })}
    </ul>
